...
- `NetBackend` trait (`crates/amber-core/src/net.rs`): the only integration point.
  ...
  - `wants_poll(&self) -> bool` — true while async host-side work is pending, so the
    run loop polls eagerly (caps the idle park at ~1 ms) instead of waiting out the
    50 ms backstop.
...
- Selection (`crates/amber/src/main.rs`, `cmd_vm`): `AMBER_NET` env —
...
- Snapshot/fork: `meta.net` records whether a net device was present; `restore_from`
  recreates it with a **fresh** backend (host state isn't snapshotted) when
  `meta.net`, keeping the device set aligned with the template. `amber template`
  builds with net so exec'd/forked sandboxes have it.
...
- **Outbound TCP**: destination NAT. On a guest SYN to `ext:port`, dial a host
  ...
  listening on the gateway's **own** IP + a fresh ephemeral port; rewrite
  guest→ext frames' dst to `gateway:eph` (IP+TCP checksums refilled) before smoltcp
  sees them, and rewrite the reply src back. (Needed because smoltcp's `any_ip`
  won't accept SYNs to a foreign destination on a listening socket.)
...
- **Inbound / port-forward**: `AMBER_PORTS="hostport:guestport,..."`. A host
  ...
  guest and bridges. (Currently the boot path; daemon would need to pass
  `AMBER_PORTS` to exec/fork workers.)
- **Latency**: `wants_poll()` is true while flows/queries/listeners exist, so
  replies reach the guest in ~ms.

**Limits (where the other backends win):** outbound is TCP + DNS only (no general
UDP NAT, no ICMP to external hosts, no IPv6); the TCP/poll path is hand-rolled
(~300 lines) and basic; performance is fine for agent tasks, not tuned for heavy
throughput. Good enough for the common case: outbound HTTP/HTTPS/DNS + inbound
forward, self-contained and rootless.
...
- **Benefits**: complete NAT for any TCP/UDP/ICMP and **IPv6**; mature and
  battle-tested; better performance under load. Covers what smoltcp does not
  (arbitrary UDP, ICMP-to-external, IPv6).
- **Costs**: an **external binary** (breaks single-binary self-containment) and a
  gvproxy process per VM (or shared).
- **Implementation** (small, ~50–80 lines): a `GvproxyBackend` that connects to the
  unix socket gvproxy serves and speaks its **vfkit/qemu framing** — each Ethernet
  frame prefixed by a length (vfkit: a 4-byte big-endian length; confirm against the
  gvproxy version). `send` writes `len ++ frame`; a reader thread reads
  `len ++ frame` and queues them for `poll`; the reader unparks/poll-flags so frames
  reach the guest promptly. Start gvproxy as a child (or require it running) with
  `--listen-vfkit unixgram://...` (check the exact flag/transport per release).
- **Testing**: install `gvproxy` (Go build or a release download), run it, point the
  backend at its socket, verify the guest reaches the internet (incl. a UDP/IPv6
  case smoltcp can't do).
- **Verdict**: add as an opt-in for users who need IPv6 / arbitrary UDP / throughput
  and accept the binary. Marginal for the core agent-sandbox case.

## vmnet — kernel NAT (not built; needs root or a helper)

Apple's `vmnet` framework (what Virtualization.framework, Docker Desktop, lima use).

- **Benefits**: NAT **accelerated by the host kernel** (faster than userspace), a
  real NIC's worth of protocols (TCP/UDP/ICMP/IPv6), and **bridged mode** (the guest
  gets its own IP on the real LAN). DHCP is provided by vmnet.
- **Costs/blockers**: vmnet shared/NAT mode requires **root** or the *restricted*
  ...
  - **(b) a `socket_vmnet`-style root helper** (the lima/colima pattern) — a small
    ...
    socket; amber connects **unprivileged**. Clean, but it's an external/extra
    privileged component to ship and audit.
- **Implementation**: link the vmnet framework (`vmnet_start_interface` in
  `VMNET_NAT`/`VMNET_BRIDGED` mode; read/write via the dispatch-callback API or
...
- **Testing**: requires `sudo` (mode a) or building the helper (mode b). Verify the
  guest gets a vmnet DHCP lease and full connectivity; test bridged mode separately.
- **Verdict**: supportable, yes. Worth it for users who want kernel-speed, full
  protocols, or bridged-onto-LAN, and accept root/helper. smoltcp already covers the
  rootless common case better for amber's purpose.

## TAP — the Linux/KVM backend (not built; belongs with M8)

`/dev/net/tun`. This is Firecracker's actual backend and the **natural companion to
the KVM backend (M8)** on arm64 Linux — not applicable on macOS.

- **Benefits**: kernel-fast L2; the standard, well-understood path; host does NAT via
  iptables/bridge (operator-configured, like Firecracker).
...
- **Implementation**: a `TapBackend` that `open`s a pre-created tap fd (or creates
  one), then `read`/`write`s raw Ethernet frames — a near-trivial `NetBackend` (the
  fd *is* the L2 wire). The same `pump_rx`/`poll` plumbing applies.
- **Testing**: needs arm64 Linux hardware with `/dev/kvm` + `/dev/net/tun` (same
  hardware M8 needs). Do it when building the KVM backend.
- **Verdict**: implement together with M8/KVM. The seam is already shaped for it.

## Decision matrix

| backend | when it wins | cost | testable here |
|---|---|---|---|
| **smoltcp** (default) | outbound HTTP/DNS + inbound forward; the agent-sandbox case | none (in-process, rootless) | yes ✓ |
| **gvproxy** | IPv6, arbitrary UDP, ICMP-to-external, throughput | external binary | only if `gvproxy` installed |
| **vmnet** | kernel-speed, all protocols, bridged-onto-LAN | root or a root helper | only with `sudo` / a helper |
| **TAP** | Linux/KVM production | Linux + `CAP_NET_ADMIN`; host NAT config | no (needs arm64 Linux) |

**Stance:** on macOS, smoltcp is the best fit for amber's thesis (self-contained,
...
we can install/validate them. TAP lands with the KVM backend. None should displace
smoltcp as the default; all plug into the unchanged `NetBackend` seam.
